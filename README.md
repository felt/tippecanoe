 ...
 * `-at` or `--use-tiny-feature-density-for-size`: When dropping or coalescing the smallest features, if a feature's size is below the "tiny polygon" threshold, use its density and not its size for dropping. This can improve the appearance of tiles in which large numbers of LineStrings have very similar sizes.
