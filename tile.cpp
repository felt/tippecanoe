...
static mvt_value find_attribute_value(const serial_feature *sf, std::string key, const std::map<std::string, layermap_entry> *merged_layermaps, std::string const &layername) {
	...
	if (key == ORDER_BY_INTERESTINGNESS) {
		mvt_value v;
		v.type = mvt_double;
		v.numeric_value.double_value = get_interestingness(sf, merged_layermaps, layername);
		return v;
	}
...
static bool order_partials(const serial_feature &a, const serial_feature &b, const std::map<std::string, layermap_entry> *merged_layermaps, std::string const &layername) {
	for (size_t i = 0; i < order_by.size(); i++) {
		mvt_value v1 = coerce_double(find_attribute_value(&a, order_by[i].name, merged_layermaps, layername));
		mvt_value v2 = coerce_double(find_attribute_value(&b, order_by[i].name, merged_layermaps, layername));
...
struct ordercmp {
	...
	ordercmp(const std::map<std::string, layermap_entry> *merged_layermaps_, const std::string &layername_) :
		merged_layermaps(merged_layermaps_), layername(layername_) { }

	bool operator()(const struct coalesce &a, const struct coalesce &b) {
		return order_partials(a.sf, b.sf, merged_layermaps, layername);
	}
};
...
struct partial {
	...
	bool coalesced = 0;  // coalesced-as-needed
	ssize_t renamed = 0;  // renumbered in border-matching
	...
	bool reduced = false;  // polygon dust / tiny polygon
	...
	partial(serial_feature &sf_, int z_, int tx_, int ty_, int line_detail_, int maxzoom_, double simplification_) {
		sf = sf_;
...
double simplify_partial(partial *p, drawvec &shared_nodes) {
	drawvec geom = p->sf.geometry;

	p->sf.geometry.clear();  // avoid keeping two copies in memory
	signed char t = p->sf.t;
	int z = p->z;
...
void *partial_feature_worker(void *v) {
	...
	for (size_t i = a->task; i < (*partials).size(); i += a->tasks) {
		...
		drawvec geom = (*partials)[i].sf.geometry;
		...
		if (t == VT_POLYGON && additional[A_GENERATE_POLYGON_LABEL_POINTS]) {
			t = (*partials)[i].sf.t = VT_POINT;
			geom = checkerboard_anchors(from_tile_scale(geom, z, out_detail), (*partials)[i].tx, (*partials)[i].ty, z, (*partials)[i].sf.label_point);
			to_tile_scale(geom, z, out_detail);
		}

		(*partials)[i].sf.index = i;
...
bool find_common_edges(std::vector<partial> &partials, int z, int line_detail, double simplification, int maxzoom, double merge_fraction) {
	...
	for (size_t i = 0; i < partials.size(); i++) {
		if (partials[i].sf.t == VT_POLYGON) {
			drawvec &g = partials[i].sf.geometry;
			drawvec out;
	...
	for (size_t i = 0; i < partials.size(); i++) {
		if (partials[i].sf.t == VT_POLYGON) {
			for (size_t k = 0; k + 1 < partials[i].sf.geometry.size(); k++) {
				if (partials[i].sf.geometry[k].op == VT_MOVETO) {
					ring++;
				}
...
struct write_tile_args {
	...
	const std::map<std::string, layermap_entry> *merged_layermaps = NULL;
	size_t pass = 0;
...
long long write_tile(FILE *geoms, std::atomic<long long> *geompos_in, char *metabase, char *stringpool, int z, const unsigned tx, const unsigned ty, const int detail, int min_detail, sqlite3 *outdb, const char *outdir, int buffer, const char *fname, FILE **geomfile, int minzoom, int maxzoom, double todo, std::atomic<long long> *along, long long alongminus, double gamma, int child_shards, long long *meta_off, long long *pool_off, unsigned *initial_x, unsigned *initial_y, std::atomic<int> *running, double simplification, std::vector<std::map<std::string, layermap_entry>> *layermaps, std::vector<std::vector<std::string>> *layer_unmaps, const std::map<std::string, layermap_entry> *merged_layermaps, size_t tiling_seg, size_t pass, unsigned long long mingap, long long minextent, double fraction, const char *prefilter, const char *postfilter, struct json_object *filter, write_tile_args *arg, atomic_strategy *strategy) {
	...
	for (line_detail = detail; line_detail >= min_detail || line_detail == detail; line_detail--, oprogress = 0) {
		...
		for (size_t seq = 0;; seq++) {
			...
			if (sf.dropped) {
				if (find_partial(partials, sf, which_partial, layer_unmaps, LLONG_MAX)) {
					...
					std::string layername = (*layer_unmaps)[sf.segment][sf.layer];
					if (order_by.size() > 0) {
						if (order_partials(sf, partials[which_partial].sf, merged_layermaps, layername)) {
		...
		for (auto layer_iterator = layers.begin(); layer_iterator != layers.end(); ++layer_iterator) {
			...
			if (order_by.size() != 0) {
				std::string layername = layer_iterator->first;
				std::sort(layer_features.begin(), layer_features.end(), ordercmp(merged_layermaps, layername));
			}
...
void *run_thread(void *vargs) {
	...
	for (task = arg->tasks; task != NULL; task = task->next) {
		...
		while (1) {
			...
			long long len = write_tile(geom, &geompos, arg->metabase, arg->stringpool, z, x, y, z == arg->maxzoom ? arg->full_detail : arg->low_detail, arg->min_detail, arg->outdb, arg->outdir, arg->buffer, arg->fname, arg->geomfile, arg->minzoom, arg->maxzoom, arg->todo, arg->along, geompos, arg->gamma, arg->child_shards, arg->meta_off, arg->pool_off, arg->initial_x, arg->initial_y, arg->running, arg->simplification, arg->layermaps, arg->layer_unmaps, arg->merged_layermaps, arg->tiling_seg, arg->pass, arg->mingap, arg->minextent, arg->fraction, arg->prefilter, arg->postfilter, arg->filter, arg, arg->strategy);
	...
	for (z = 0; z <= maxzoom; z++) {
		...
		for (size_t pass = 0;; pass++) {
			...
			for (size_t thread = 0; thread < threads; thread++) {
				...
				args[thread].merged_layermaps = &merged_layermaps;
